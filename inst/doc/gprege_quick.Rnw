...
%\url{http://staffwww.dcs.shef.ac.uk/people/A.Kalaitzis/DellaGattaData.RData}
\url{https://gprege.googlecode.com/svn/trunk/DellaGattaData.RData}
and can be loaded using
<<eval=FALSE>>=
  # Download full Della Gatta dataset.
  load(url('https://gprege.googlecode.com/svn/trunk/DellaGattaData.RData'))
@
...
<<>>=
  # Download full Della Gatta dataset.
  con <- url('https://gprege.googlecode.com/svn/trunk/DellaGattaData.RData')
  load(con)#  close.connection(con)
...
<<>>=
  ...
  for (i in 1:3) { 
    # Read gene numbers.
    tmp <- read.table(url(paste(
      'http://arxiv.org/src/1106.4333v1/anc/DGdat_p63_case',
      i,'_GL.txt',sep='')), skip=1)
    genenumbers <- as.numeric(lapply( as.character(tmp[,2]), 
                                      function(x) x=substr(x,2,nchar(x))))
